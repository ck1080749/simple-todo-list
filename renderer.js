...
//load page
todoList = window.electronAPI.loadFile()
...
$('#new-todo').on('keydown', function(e){//e: pressed key
  if(e.which == ENTER_KEY){
    ...
    if(filter0.test(val) && filter1){
      todoList.append({val:val, com:false, dl: deadLine.toString(), _id: todoList.length});//TODO:what is id for?
      $('#todo-list').append(template(val, false, todoList[todoList.length-1]._id, deadLine.toString()));
      window.electronAPI.updateFile(todoList);
      $(this).val('');
      count();
    }
  }
...
$('btn1').on('click', function(){//輸入一
  //TODO: for unknown reason, if you edit something when it's completed, the font won't change to normal until reload the page
    ...
    if(filter0.test(val) && filter1){
      todoList.append({val:val, com:false, dl: deadLine.toString(), id:todoList.length});
      $('#todo-list').append(template(val, false, todoList[todoList.length-1]._id, deadLine.toString()));
      window.electronAPI.updateFile(todoList);
      $('#new-todo').val('');
      count();
...
$('#todo-list').on('click', 'button', function(){//delete
  ...
  window.electronAPI.updateFile(todoList);  //tdCollection.save();  
  count();
});
